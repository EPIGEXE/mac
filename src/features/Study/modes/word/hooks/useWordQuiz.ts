...
export function useWordQuiz({ noteId, noteContent, noteTitle, noteType }: UseWordQuizProps) {
    ...
    // 클라이언트에서 blindedContent 생성
    const blindedContent = useMemo(() => {
        if (!quizData?.blanks || quizData.blanks.length === 0) return ''
        return createBlindedContent(noteContent, quizData.blanks)
    }, [noteContent, quizData?.blanks])
